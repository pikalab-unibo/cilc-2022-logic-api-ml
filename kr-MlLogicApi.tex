...
\title{Logic Programming library for Machine Learning: API design and prototype}
...
$^{2}$matteo.castigliò@studio.unibo.it
}
...
%
We believe the bottleneck is caused by the lack of suitable technologies enabling and easing the experimentation of integrated or hybrid systems.
%
...
%
The contribution is twofold: \textit{i)} let logic programmers exploit the benefits of sub-symbolic AI, and, in particular, neural networks; and \textit{ii)} enable the practical experimentation of hybrid systems---involving both logic and neural processing of data.
%
Accordingly, We discuss the design and prototype of a logic based API for machine learning.
%
...
Technically, we prototype our API via a logic library -- namely, the \mllib{} \cite{xxx} -- targetting the \twopkt{} ecosystem \cite{xxx}, and the JVM platform.
%
...
%
\deeplearningforj{} \cite{xxx} is the underlying library we leverage upon in this paper.
%
...
We argue that our work represents the first step towards a wider degree of interoperability among symbolic- and sub-symbolic AI.
%
...
To properly design a logic library for dealing with hybrid reasoning, some basic goals to achieve should be taken into account: enable hybrid reasoning, full support of declarative ML, enable the exploitation of symbolic data sources (in addition to the others), make it possible to select a model via resolution.
%
...
\emph{Declarative ML.} Declarative ML is a paradigm by which data scientists' code should only specify \emph{what} an ML workflow should do, by leaving the underlying platform in charge to understand \emph{how}.
%
This is partially supported by the current practice of data science which relies on high-level languages (e.g. Python) and libraries of elementary components to be composed (e.g. Scikit-Learn).
%
...
%
To support the declarative expression of an ML workflow in the LP framework a new logic API is so required.
%
...
%----------------------------------------------------------------------------------------
\section{Logic API for ML: requirements}
\label{sec:requirements}
%----------------------------------------------------------------------------------------

To support the aforementioned goals, logic APIs must cover the full gamma of tasks involved in any possible ML workflow, detailed and discussed in the following.
...
%%
%Thus, from a coarse-grained perspective, a machine learning workflow can be conceived as composed by six major phases, detailed in the following.

\emph{Dataset loading.}
...
\emph{Predictor selection and definition.}
...
In this section we define the meta-model leading the design, implementation, and usage of our logic API for ML accordingly to the goals and the requirements discussed above.

The entities to be considered in order to define the library meta-model are the entities that required to fully describe the ML workflow:
%
\begin{itemize}
	...
	%
	\item\emph{Schema:} %per rappresentare il formato dei dati iniziale e dei dati processati.
	...
	%
	\item \emph{Dataset:} %per rappresentare il dataset iniziale, il dataset processato, il training set, il test set ed il validation set.
	a collection of values matching a particular schema (supposed to be known).
	%
	\item\emph{Transformation:} %per rappresentare una sequenza di operazioni di elaborazione.
	...
	%
	\item\emph{Predictor:} %per rappresentare il predittore nelle fasi di definizione, addestramento valutazione e uso
...
%
Each phase of the ML workflow is characterised by a specific set of activities that should be declaratively representable in logic.
%
Accordingly, in the remainder of this section, following the ML workflow elicited in \cref{sec:requirements}, the activities the logic API should support -- along with the entities they operate upon -- are discussed.
...
%
\begin{itemize}
    \item Dataset loading: operation of loading a dataset from either a value -- representing either a local or remote file --, or from a Prolog theory
    \item Schema declaration:  operation of constructing a representation for a given schema
    \item Target features declaration: operation of tagging a portion of features of some schema as either inputs or outputs (a.k.a. targets)
    \item Dataset splitting: operation of horizontally partitioning a dataset into two or more smaller datasets.
\end{itemize}

\emph{Dataset pre-processing}
%
...
%
\begin{itemize}
    \item Transformation declaration: operation of declaratively encoding a transformation operation to be applied to all data in a dataset
    ...
    \item Pipeline composition: operation of declaratively constructing a composite transformation as a cascade of simpler transformations
    \item Transformation application: operation of actually constructing a new dataset from a prior dataset and a transformation
\end{itemize}
...
%
\begin{itemize}
    \item Predictor declaration: operation of constructing a representation for a particular predictor, which implies choosing the predictor family and specifying actual values for its hyper parameters
\end{itemize}
%%
...
%%
\begin{itemize}
    \item Predictor fitting w.r.t. a training set of data: operation of fitting a predictors' internal parameter on some provided training data
\end{itemize}
%%
...
%%
\begin{itemize}
    \item Predictor querying: operation where (possibly unseen) values are provided to some trained predictor as a query, and the resulting values are interpreted as predictions
\end{itemize}
...
%
\begin{itemize}
    \item Predictor scoring: operation of computing a scoring value out of a trained predictor, a test dataset, and a scoring function
\end{itemize}
...
At the functional level, the design of the \mllib{} is provided in terms of logic predicates acting on the above defined entities. Details about the predicates are discussed in the Appendix.
%
...
In the following the usage of the \mllib{} to serve the purposes described in \cref{sec:goals} is discussed.
...
From an ML perspective, our examples assume a very simple scenario where a neural-network classifier is trained on the well known Iris dataset\footnotemark.
%
...
%RC: questo listting va assolutamente ridotto, serve tutta sta roba?
\lstinputlisting[
    float,
    language=Prolog,
    label={lst:training},
    caption={ML workflow declarative description for selecting the best hyper and learning parameters for a NN}
...
\begin{center}
    \begin{tabular}{l}
        \pl{row(+Dataset: \pli{ref}, ?Index: \pli{integer},}\\ 
        \pl{-Values: \pli{list}).}
        \\
        \pl{column(+Dataset: \pli{ref}, ?Attribute:} \\
        \pl{{\pli{integer}|\pli{atom}, -Values: \pli{list}).}}
        \\
        \pl{cell(+Dataset: \pli{ref}, ?Index:}\\
        \pl{\pli{integer},  ?Attribute: \pli{integer}|\pli{atom}, }\\
        \pl{-Values: \pli{list}).}
    \end{tabular}
\end{center}
}
...
\begin{center}
    \begin{tabular}{l}
        \pl{input\_layer(+Size: \pli{integer}, -Layer:} \\
        \pl{\pli{ref}).}
        \\
        \pl{$\langle type \rangle$\_layer(+Previous: \pli{ref}, +Size:}\\
        \pl{ \pli{integer}, +Activation: \pl{ref}, -Layer:}\\
        \pl{ \pli{ref}).}
    \end{tabular}
\end{center}}
...
{\footnotesize
    \begin{tabular}{l}
        \pl{activation(\pli{identity}).}  \\
        	\qquad denoting $f(x) = x$
        \\
        \pl{activation(\pli{sigmoid}).} \\
        \qquad denoting $f(x) = 1/(1+e^{-x})$
        \\
        \pl{activation(\pli{tanh}).} \\
        \qquad denoting $f(x) = \mathit{tanh}(x)$
        \\
        \pl{activation(\pli{relu}).} \\
        \qquad denoting $f(x) = \mathit{max}(0, x)$
    \end{tabular}
}
